...
from models import GameState, Killer, Survivor, Alias
...
class PerkGame(commands.Cog):
    ...
    async def on_message(self, message):
        if (
            ...
            if message.content == 'give_up':
                perks = [f"{x.name}: {x.popularity}%" for x in self.game_state.character.perks]
            ...
            if characters.count() > 1:
                await message.channel.send("Your guess returned too many characters, please be more specific")
            ...
            if character:
                if self.game_state.guess(character.name):
                    ...
                    await message.channel.send(f"{character.name} was not the correct answer.")
            else:
                await message.channel.send("No character with such name, no attempt counted")
    ...
    async def guess_perks(self, ctx, game_type=GameState.RANDOM):
        ...
        if not self.game_state.is_game_active:
            ...
            perks_popularity = [str(x.popularity) + '%' for x in self.game_state.character.perks]
            await thread.send(f"{type(self.game_state.character).__name__} with perk pick rates: {', '.join(perks_popularity)}")
